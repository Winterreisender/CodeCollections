...
sem_t not_writing_ready[WRITER_NUM]; //between rd and wr
...

{
	...
	for(int i=0;i<20;i++){
		...
		printf("Writer %d waiting not_writing_ready[%d] \n", id,id);

		sem_wait(&not_writing_ready[id]);



		printf("Writer %d ready. waiting write_lock \n", id);
...

{
	...
	for(int i=0;i<20;i++){
		...
		printf("Reader %d ready. waiting no writers ready \n",id);

		for (int i=0; i<READER_NUM; i++)

			sem_wait(&not_writing_ready[i]);
...

{
	...
	for(int i=0;i<WRITER_NUM;i++)
		...
		sem_init(&not_writing_ready[i],0,1);
